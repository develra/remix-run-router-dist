...
(function (global, factory) {
  ...
  function createRouter(init) {
    ...
    function handleShouldBlockTrackers(blockedKey, blockedInfo) {
      if (!blockedInfo || blockedInfo.shouldBlockTrackers.length === 0) {
        return false;
      }

      if (blockedInfo.shouldBlockTrackers.some(tracker => tracker.hasResolved === false)) {
        return false;
      }

      if (blockedInfo.shouldBlockTrackers.some(tracker => tracker.shouldBlock)) {
    ...
    function handleBlockers(_action, location, delta) {
      ...
      for (let [key, blocker] of state.blockers) {
        ...
        } = blocker.fn(); // If a POP navigation occurs while a navigation is blocked, one of
        ...
        if (blocker.state === "unblocked") {
          ...
          if (blockedInfo && blockedInfo.shouldBlockNextNav) {
            let revertDelta = blockedInfo.revertStackIndex - init.history.state.idx; // There are situations where we have another pop state but
          ...
          let shouldBlockTrackerIndex = blockedInfo.shouldBlockTrackers.length; // begin tracking what happens with shouldBlock
          ...
          blocked.set(key, blockedInfo); // The function-of-truth! here is where the black-box of potential states really begins.

          if (shouldBlock()) {
            blockedInfo = blocked.get(key);

            if (blockedInfo && blockedInfo.shouldBlockTrackers[shouldBlockTrackerIndex]) {
            ...
            if (!skipStateUpdate) {
              setBlockerState(key, "blocked", {
                ...
                onReset() {// noop, we've already blocked and state will be updated to
                  // `unblocked`
                }
            ...
            if (!shouldProceed) {
              ...
              targetLocation = ((_blockedInfo = blockedInfo) == null ? void 0 : _blockedInfo.targetLocation) || location;
            }
          }
        }
      ...
      blocked = new Map(); // Good to go - let the navigation function know it can proceed and where to go.
    ...
    function initialize() {
      ...
      unlistenHistory = init.history.listen(_ref => {
        ...
        let handleBlockersResult = handleBlockers(historyAction, location, delta); // No blockers so we are GOOD TO GO 🎉🚀

        if (handleBlockersResult.shouldProceed) {
          return startNavigation(historyAction, handleBlockersResult.targetLocation || location);
        }
    ...
    function getBlocker(key, fn) {
      ...
      if (fn) {
        if (typeof fn === "function") {
          ...
          warning(false, "A blocker function update was requested with a value that is not a function. This is not allowed, and the blocker's function will not be updated.");
        }
      }
    ...
    function setBlockerState(key, nextState, opts) {
      let blocker = state.blockers.get(key);
      ...
      invariant(nextState === "proceeding" || nextState === "blocked" || nextState === "unblocked", "Invalid blocker state: " + nextState); // If the blocker state changes we need to update the router state to notify
      ...
      if (nextState === "blocked") {
        ...
        blocker.proceed = async () => {
          ...
          await (onProceed == null ? void 0 : onProceed());
        };
    ...
    async function startNavigation(historyAction, location, opts) {
      ...
      if (Array.from(state.blockers).some(_ref2 => {
        let [_, blocker] = _ref2;
    ...
    async function handleFetcherAction(key, routeId, path, match, requestMatches, submission) {
      ...
      revalidatingFetchers.filter(_ref6 => {
        let [staleKey] = _ref6;
        return staleKey !== key;
      }).forEach(_ref7 => {
        let [staleKey] = _ref7;
      ...
      fetchControllers.delete(key);
      revalidatingFetchers.forEach(_ref8 => {
        let [staleKey] = _ref8;
    ...
    async function callLoadersAndMaybeResolveData(currentMatches, matches, matchesToLoad, fetchersToLoad, request) {
      ...
      // accordingly
      let results = await Promise.all([...matchesToLoad.map(match => callLoaderOrAction("loader", request, match, matches, router.basename)), ...fetchersToLoad.map(_ref9 => {
        let [, href, match, fetchMatches] = _ref9;
        return callLoaderOrAction("loader", createClientSideRequest(href, request.signal), match, fetchMatches, router.basename);
      })]);
      let loaderResults = results.slice(0, matchesToLoad.length);
      let fetcherResults = results.slice(matchesToLoad.length);
      await Promise.all([resolveDeferredResults(currentMatches, matchesToLoad, loaderResults, request.signal, false, state.loaderData), resolveDeferredResults(currentMatches, fetchersToLoad.map(_ref10 => {
        let [,, match] = _ref10;
        return match;
